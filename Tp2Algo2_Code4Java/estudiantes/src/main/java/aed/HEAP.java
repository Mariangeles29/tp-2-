...
class HEAP<T extends Comparable<T>> {
    ...
    public void buildHeap(ArrayList<T> elements, ArrayList<Handle<T>> newHandles) {
    this.datos = new ArrayList<>(elements);
    this.handles = new ArrayList<>(newHandles);
    
    // Actualizar posiciones de los handles
    for (int i = 0; i < handles.size(); i++) {
        handles.get(i).setPosicion(i);
    }
